...
CallbackReturn UFRobotFakeSystemHardware::on_init(const hardware_interface::HardwareInfo& info)
{
  ...
  for (int i = 0; i < info_.joints.size(); i++)
  {
    ...
    RCLCPP_INFO(LOGGER, "Joint '%s' has index %d",
  ...
  for (const hardware_interface::ComponentInfo& joint : info_.joints)
  {
    ...
    for (auto i = 0u; i < joint.state_interfaces.size(); ++i)
    {
      if (joint.state_interfaces[i].name == hardware_interface::HW_IF_POSITION)
      {
        ...
        if (!joint.state_interfaces[i].initial_value.empty())
        {
          position_cmds_[i] = std::stod(joint.state_interfaces[i].initial_value);
        }
        else
        {
          // Initialize the value in old way with warning message
          auto it2 = joint.parameters.find("initial_" + joint.state_interfaces[i].name);
          if (it2 != joint.parameters.end())
          {
            position_cmds_[i] = std::stod(it2->second);
            RCUTILS_LOG_WARN_NAMED("fake_generic_system",
...
hardware_interface::return_type UFRobotFakeSystemHardware::read(const rclcpp::Time& time,
...
hardware_interface::return_type UFRobotFakeSystemHardware::write(const rclcpp::Time& time,
                                                                 const rclcpp::Duration& period)
{
  // std::string pos_str = "[ ";
  // std::string vel_str = "[ ";
